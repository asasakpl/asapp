  ...
  <v-container>
    <v-row>
      <v-col align="center" class="mx-auto px-auto">
        <v-btn @click="$router.push('/m35')" icon fab class="ml-2">
      ...
      <v-col cols="11" class="mx-auto px-auto">
  ...
  methods: {
    getInfo() {
      axios
        ...
        .then((res) => {
          this.load = true
          for (let x in res.data.m35.members) {
            if (res.data.m35.members[x].subscribed == true) {
              res.data.m35.members[x].subscribed = 'Tak'
            } else {
              res.data.m35.members[x].subscribed = 'Nie'
            }
          }
          this.members = res.data.m35.members
          this.error = false
          this.load = false
        })
        .catch((err) => {
    ...
    removeSubscriber() {
      axios
        .post('/mail/cancel', { email: this.email })
        .then((res) => {
          this.load = true
          this.dialog = false
