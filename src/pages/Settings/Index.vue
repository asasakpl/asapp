      ...
      <v-col>
        <v-row class="justify-left pl-5">
          <v-card-title>
            ...
            <v-flex xs6>
              <v-select single-line bottom v-model="theme" :items="themes" @change="setTheme">
                <option v-for="(theme, i) in themes" :key="`Lang${i}`" :value="theme">{{ theme }}</option>
              </v-select>
            </v-flex>
          </v-card-title>
      ...
      <v-row class="justify-left pl-5">
        <v-col>
          <v-card-title>
            ...
            <span class="ml-2">{{ electron }}</span>
          </v-card-title>

          <v-card-title>
            <v-icon color="green">mdi-nodejs</v-icon>
            <span class="ml-2">{{ node }}</span>
          </v-card-title>
          <v-card-title>
